import {
  ...
  PRODUCT_EDIT_UPDATE_FAIL,
  PRODUCT_EDIT_UPDATE_SUCCESS,
} from "../types";
...
export const updateProduct = (
  ...
  try {
    ...
    // Update Product
    const updateProduct = await axios.put(`/api/product/update?productID=${productId}`, productInfo, {
      headers: {
        "Content-Type": "application/json",
        authorization: `Bearer ${accesstoken}`,
      },
    ...
    dispatch({
      type: PRODUCT_EDIT_UPDATE_SUCCESS,
      payload: updateProduct.data,
    });
  } catch (err) {
    dispatch(returnErrors(err.response.data, err.response.status));
    dispatch({ type: PRODUCT_EDIT_UPDATE_FAIL });
  }
};
